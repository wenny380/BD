...
INDEX `fk_idFaculty_idMajor` (`idFaculty` ASC, `idMajor` ASC) VISIBLE,
...
insert into Groupx (idGroup,idStudent,yearx,idFaculty,idMajor)values
(1,2,2017,1,1),
(2,3,2019,1,2),
(3,1,2018,2,1);
...
(1,'2019-11-11',2,'Math calculus', 2),
(2,'2019-11-11',2,'Math calculus2', 3),
(3,'2018-11-11',2,'Math calculus3', 1);
...
CREATE PROCEDURE Getteacher1(IN lastN VARCHAR(255))
BEGIN
SELECT * FROM Teacher WHERE lastname = lastN;
END //

CALL Getteacher1('Montezima'); 


CREATE PROCEDURE Getstudent1(IN lastN VARCHAR(255))
BEGIN
SELECT * FROM Student WHERE lastName = lastN;
END //

CALL Getstudent1('Wen'); 


/*USE teamFourDb;
CREATE TRIGGER add_Teacher BEFORE UPDATE ON Student
FOR EACH ROW  
BEGIN
/*declare txt varchar(255);
UPDATE Teacher
SET firstName='woooooo' WHERE idTeacher=1;
/*INSERT INTO Student
    SET firstName = 'STERN',
...
USE teamFourDb;
DELIMITER //
CREATE TRIGGER schedulex BEFORE INSERT ON Conduct_Class
FOR EACH ROW  
BEGIN
DECLARE var VARCHAR(255);
IF (SELECT idGroup FROM Groupx WHERE Groupx.idGroup = new.idGroup AND Groupx.idMajor = new.idMajor)
THEN
SET var='ok';
ELSE 
...
INSERT INTO Conduct_Class(idConductClass,idTeacher,semester,yearx,idFaculty,idMajor,idSubject,idClassCategory, idGroup)values
(7,1,1,2017,1,1,1,2,1);

SELECT * FROM Conduct_class;
